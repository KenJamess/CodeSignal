...
BEGIN
	...
	SELECT a.id,
        CASE
            WHEN SUM(e.expenditure_sum) > a.value THEN SUM(e.expenditure_sum) - a.value
            ELSE 0
        END AS loss
    FROM allowable_expenditure a
    LEFT JOIN expenditure_plan e
    ON WEEK(e.monday_date, 1) BETWEEN a.left_bound AND a.right_bound
    GROUP BY a.id
    ORDER BY a.id;
END