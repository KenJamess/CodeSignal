...
BEGIN
    ...
    SELECT a.bug_title,
           a.component_title,
           (SELECT count(*)
            FROM BugComponent bug
            WHERE a.component_id = bug.component_id
            GROUP BY bug.component_id) as bugs_in_component
    FROM (SELECT b.title AS bug_title, c.title AS component_title, c.id as component_id
          FROM Bug b
                   JOIN BugComponent bc ON b.num = bc.bug_num
                   JOIN Component c ON bc.component_id = c.id
          WHERE b.num IN (SELECT bug_num FROM BugComponent GROUP BY bug_num HAVING COUNT(DISTINCT component_id) > 1)
          GROUP BY c.id, b.title
          ORDER BY c.id, b.num) as a
