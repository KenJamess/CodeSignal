...
BEGIN
    ...
    FROM (SELECT id, 'name' AS column_name, name AS value, 1 AS order_column
          FROM workers_info
          WHERE name IS NOT NULL
          UNION
          SELECT id, 'date_of_birth' AS column_name, DATE_FORMAT(date_of_birth, '%Y-%m-%d') AS value, 2 AS order_column
          FROM workers_info
          WHERE date_of_birth IS NOT NULL
          UNION
          SELECT id, 'salary' AS column_name, salary AS value, 3 AS order_column
          FROM workers_info
          WHERE salary IS NOT NULL) AS union_result
    ORDER BY id, order_column;
END