...
BEGIN
    ...
    SELECT ROUND(SUM(SQRT(POW(x - (SELECT x FROM cities WHERE id = c.id + 1), 2) + POW(y - (SELECT y FROM cities WHERE id = c.id + 1), 2))), 9) AS total
    FROM cities c
    WHERE id < (SELECT MAX(id) FROM cities);
END