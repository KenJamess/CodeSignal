...
BEGIN
    ...
    SELECT e.event_id,
        IF(s.hours = 24, DATE_FORMAT(DATE_ADD(e.date, INTERVAL s.timeshift MINUTE), '%Y-%m-%d %H:%i'),
            DATE_FORMAT(DATE_ADD(e.date, INTERVAL s.timeshift MINUTE), '%Y-%m-%d %h:%i %p')) AS formatted_date
    FROM events e
    JOIN settings s
