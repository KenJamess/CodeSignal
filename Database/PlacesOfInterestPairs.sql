...
Given the sights table, find all pairs of places that are less than 5 km from each other and return them as a table with the columns place1 and place2.
...
BEGIN
    ...
    SELECT CASE WHEN s1.name < s2.name THEN s1.name ELSE s2.name END AS place1,
           CASE WHEN s1.name < s2.name THEN s2.name ELSE s1.name END AS place2
    FROM sights s1
             JOIN sights s2
                  ON s1.id < s2.id
